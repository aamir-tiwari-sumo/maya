...
func (k *KubernetesClient) Get(name string) (*CStorPool, error) {
	d, err := k.Clientset.OpenebsV1alpha1().CStorPools().Get(context.TODO(), name, v1.GetOptions{})
...
func (k *KubernetesClient) List(opts v1.ListOptions) (*CStorPoolList, error) {
	dl, err := k.Clientset.OpenebsV1alpha1().CStorPools().List(context.TODO(), opts)
...
func (k *KubernetesClient) Create(diskObj *apis.CStorPool) (*CStorPool, error) {
	d, err := k.Clientset.OpenebsV1alpha1().CStorPools().Create(context.TODO(), diskObj, v1.CreateOptions{})
	return &CStorPool{d, nil}, err
}
