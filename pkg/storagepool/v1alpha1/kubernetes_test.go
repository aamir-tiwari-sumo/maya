...
func fakeListOk(cs *clientset.Clientset,
	...
	return &apis.StoragePoolList{}, nil
}
...
func TestWithClientset(t *testing.T) {
	...
	for name, mock := range tests {
		...
		mock := mock // pin it
		t.Run(name, func(t *testing.T) {
			o := WithClientset(mock.clientSet)
			fake := &Kubeclient{}
			o(fake)
...
func TestKubernetesCreate(t *testing.T) {
	tests := map[string]struct {
		...
		"When creating resource throws error": {
			...
			fakeCreateErr,
			true},
		"When StoragePool object is nil": {
			nil,
			fakeGetClientsetOk,
			fakeCreateOk,
			true},
		"When an empty StoragePool struct is given": {
			...
			fakeCreateOk,
			false},
		"When non-empty StoragePool struct is given": {
			&apis.StoragePool{
				ObjectMeta: metav1.ObjectMeta{Name: "sp-cjed"},
			},
			fakeGetClientsetOk,
			fakeCreateOk,
			false},
...
func TestKubernetesDelete(t *testing.T) {
	...
	for name, mock := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			err := k.Delete(mock.sp, &metav1.DeleteOptions{})
